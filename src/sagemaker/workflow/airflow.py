...
def processing_config(
    ...
    Args:
        ...
        kms_key_id (str): The AWS Key Management Service (AWS KMS) key that Amazon SageMaker
            uses to encrypt the processing job output. KmsKeyId can be an ID of a KMS key, ARN of a KMS key,
            alias of a KMS key, or alias of a KMS key. The KmsKeyId is applied to all outputs.
    ...
    if job_name is not None:
        ...
        processor._current_job_name = (
            utils.name_from_base(base_name) if base_name is not None else utils.base_name_from_image(processor.image_uri)
        )

    input_dict = []
    for input in inputs:
        input_dict.append(input._to_request_dict())
    config = {
    ...
    if experiment_config is not None:
        config["ExperimentConfig"] = processor.experiment_config
